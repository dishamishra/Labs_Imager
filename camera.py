...
class Camera:
    ...
    def open(self,  roi_shape=(1280, 1024), roi_pos=(0,0), exposure=10., framerate=1.0):
        ...
        if out == 0:
            ...
            raise uc480Error(str(out), "Opening the ThorCam failed with error code ", 'is_InitCamera')
    ...
    def close(self):
        if self.meminfo != None:
            self.clear_image()
        if self.hid != None:
            out = self.call('is_ExitCamera', self.hid, argtypes=[c_int])
            if out == 0:
                print("ThorCam closed successfully.")
            else:
                print("Closing ThorCam failed with error code "+str(out))
