...
class Pololu:
    ...
    def __init__(self, chs=[0, 1, 2, 3, 4, 5]):
        if Pololu.count > 0:
            ...
            self._default_max = [1500, 2496, 2496, 2496, 19999, 2496]
    ...
    def set_position(self, ch, target, blocking=False, interval=0.5):
        if target < self._default_min[ch]:
            warnings.warn(f'Requested minimum exceeds range. Minimum set to default {self._default_min[ch]}.')
            target = self._default_min[ch]
        if target > self._default_max[ch]:
            warnings.warn(f'Requested maximum exceeds range. Maximum set to default {self._default_max[ch]}.')
            target = self._default_max[ch]
